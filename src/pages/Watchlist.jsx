...
const Watchlist = () => {
  ...
  <div className="min-h-screen bg-linear-to-b from-[#2f2f2f] via-[#111] to-[#141414] text-white">
    ...
    <div className="max-w-7xl mx-auto pt-24 sm:pt-28 pb-16 px-4 sm:px-0">
      {/* ===== HEADER ===== */}
      <div className="mb-5">
        <h1 className="text-2xl sm:text-4xl font-bold text-[#F6E7C6] anton mt-2 mb-2">
          Your Watchlist
        </h1>

        <p className="text-[#F6E7C6] mt-1 text-sm sm:text-xl [word-spacing:0.2rem] reem-kufi">
      ...
      {/* ===== TABS ===== */}
      <div className="flex gap-6 border-b border-black/20 mb-6 sm:mb-10 poppins-regular overflow-x-auto no-scrollbar">
        {TABS.map((tab) => (
          <button
            key={tab.key}
            onClick={() => setActiveTab(tab.key)}
            className={`relative pb-3 text-sm sm:text-md font-semibold transition whitespace-nowrap ${
              activeTab === tab.key
                ? "text-[#FF7A1A]"
      ...
      {loading ? (
        <div className="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-4 sm:gap-6">
          {Array.from({ length: 10 }).map((_, i) => (
            ...
            >
              <div className="w-full h-[180px] sm:h-[220px] md:h-[300px] bg-slate-700/40" />
            ...
            return (
              ...
              >
                ...
                <button
                  ...
                  className="absolute top-2 right-2 z-20 w-9 h-9 sm:w-8 sm:h-8 rounded-full
                ...
                >
                  ...
                  <div className="px-3 py-2">
                    <p className="text-base sm:text-xl text-[#222] anton truncate">
                      {item.title}
                    </p>

                    <p className="text-[12px] sm:text-[14px] text-[#222] capitalize">
                      {item.mediaType}
                    </p>
                    ...
                    {activeTab === "continue" &&
                      ...
                      item.progress?.lastWatched && (
                        <p className="mt-0.5 text-[12px] text-[#ed6300] poppins-semibold leading-tight">
